...
@onready var paint_array = [RedBorder, BlueBorder, YellowBorder]
...
func _process(delta):
	if not (Input.is_action_pressed("left_click")):
		if (Input.is_action_just_pressed("red_paint")):
			...
			change_active(2)
		elif (Input.is_action_just_pressed("change_paint_left")):
			shift_active_left()
		elif (Input.is_action_just_pressed("change_paint_right")):
			shift_active_right()
